...
(()=>{
    const e = new Error("Cannot find module '../db.js'");
    e.code = 'MODULE_NOT_FOUND';
    throw e;
})();
;
...
================================ */ router.get("/overview", async (req, res)=>{
    ...
    `);
        res.json({
            invoices: sales.invoices_count || 0,
            totalSales: sales.total_sales || 0,
            products: products.count_products || 0,
            totalQty: products.total_qty || 0
        });
    } catch (err) {
        console.error("GET /reports/overview error:", err);
        res.status(500).json({
            message: "خطأ في تحميل تقرير النظرة العامة"
        });
    }
});
/* ================================
...
================================ */ router.get("/sales", async (req, res)=>{
    ...
    `);
        ...
        res.status(500).json({
            message: "خطأ في تحميل تقرير المبيعات"
        });
    }
});
/* ================================
...
================================ */ router.get("/stock", async (req, res)=>{
    ...
    `);
        ...
        console.error("GET /reports/stock error:", err);
        res.status(500).json({
            message: "خطأ في تحميل تقرير المخزون"
        });
    }
});
/* ================================
   📌 4) تقرير ربحية المنتجات
================================ */ router.get("/profit", async (req, res)=>{
    ...
    `);
        const result = rows.map((p)=>{
            const totalCost = (p.purchase_price || 0) * p.quantity;
        ...
        console.error("GET /reports/profit error:", err);
        res.status(500).json({
            message: "خطأ في تحميل تقرير الربحية"
        });
    }
});
/* ================================
    ...
    `);
        ...
        rows.forEach((p)=>{
            if (p.expiry_date) {
                ...
                else if (exp <= monthAhead) nearExpiry.push(p);
            }
            if (p.quantity <= p.min_qty) lowStock.push(p);
        });
        res.json({
            expired,
            nearExpiry,
...
================================ */ router.get("/shifts", async (req, res)=>{
    try {
      ...
      LEFT JOIN shift_totals st ON st.shift_id = s.id
      ORDER BY s.opened_at DESC
    `);
        res.json(shifts);
    } catch (err) {
        console.error("GET /reports/shifts error:", err);
        res.status(500).json({
            message: "خطأ في تحميل تقرير الشفتات"
        });
    }
});
/* ================================
   📌 7) المنتجات الأكثر مبيعاً
      — هنا التعديل الذي طلبته —
================================ */ router.get("/top-products", async (req, res)=>{
    ...
    `);
        ...
        res.status(500).json({
            message: "خطأ في تحميل أكثر المنتجات مبيعاً"
        });
    }
});
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1fe4edff._.js.map